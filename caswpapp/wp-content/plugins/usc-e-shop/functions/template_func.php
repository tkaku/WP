...
function usces_the_itemPrice_taxincluded( $out = '' ) {
	...
	$tax_rate = $usces_tax->get_sku_tax_rate( $post->ID, $usces->itemsku['code'] );
	$tax = (float)sprintf( '%.3f', (float)$usces->itemsku['price'] * (float)$tax_rate / 100 );
	$tax = usces_tax_rounding_off( $tax );
	$price = $usces->itemsku['price'] + $tax;
...
function usces_the_itemCprice_taxincluded( $out = '' ) {
	...
	$tax_rate = $usces_tax->get_sku_tax_rate( $post->ID, $usces->itemsku['code'] );
	$tax = (float)sprintf( '%.3f', (float)$usces->itemsku['cprice'] * (float)$tax_rate / 100 );
	$tax = usces_tax_rounding_off( $tax );
	$price = $usces->itemsku['cprice'] + $tax;
