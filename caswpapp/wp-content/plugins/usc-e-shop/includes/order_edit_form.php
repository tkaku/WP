...
<?php if( $reduced_taxrate ): ?>
	<?php if( !$usces_tax->was_reduced_taxrate_before( $order_id ) ): ?>
...
<table cellspacing="0" id="cart_table">
	...
	<tr>
		...
		<th class="productname"><?php _e('Items','usces'); ?></th>
...
<?php if( $reduced_taxrate ): ?>
	<?php if( 'products' == $tax_target ):
		...
		<tr>
			<td colspan="5" class="aright"><?php printf( __( "%s%% consumption tax", 'usces' ), $usces_tax->tax_rate_standard ); ?><span id="include_tax_standard"><?php echo $include_tax_standard; ?></span></td>
			<td class="aright"><input name="order_tax_standard" id="order_tax_standard" type="text" class="text price" value="<?php if( !empty( $order_tax_standard ) ) { usces_crform( $order_tax_standard, false, false, '', false ); } else { echo '0'; } ?>"<?php echo $tax_readonly; ?> /></td>
			<td colspan="2"><?php if( 'exclude' == $tax_mode ) echo __( 'It will be not caluculated automatically.', 'usces' ); ?>&nbsp;</td>
		</tr>
		<tr>
			<td colspan="5" class="aright"><?php printf( __( "%s%% consumption tax", 'usces' ), $usces_tax->tax_rate_reduced ); ?><span id="include_tax_reduced"><?php echo $include_tax_reduced; ?></span></td>
			<td class="aright"><input name="order_tax_reduced" id="order_tax_reduced" type="text" class="text price" value="<?php if( !empty( $order_tax_reduced ) ) { usces_crform( $order_tax_reduced, false, false, '', false ); } else { echo '0'; } ?>"<?php echo $tax_readonly; ?> /></td>
			<td colspan="2"><?php if( 'exclude' == $tax_mode ) echo __( 'It will be not caluculated automatically.', 'usces' ); ?>&nbsp;</td>
		</tr>
		<tr>
...
<?php endif; ?>
		<tr>
			...
			<td class="aright"><input name="offer[usedpoint]" id="order_usedpoint" class="text price red" type="text" value="<?php if( isset($data['order_usedpoint']) && !empty($data['order_usedpoint']) ) {echo esc_attr($data['order_usedpoint']); } else { echo '0'; } ?>" /></td>
			<td><?php _e('granted points', 'usces'); ?></td>
			<td class="aright"><input name="offer[getpoint]" id="order_getpoint" class="text price" type="text" value="<?php if( isset($data['order_getpoint']) && !empty($data['order_getpoint']) ) {echo esc_attr($data['order_getpoint']); } else { echo '0'; } ?>" /></td>
		</tr>
<?php if( $reduced_taxrate && 'all' == $tax_target ): ?>
		<tr>
			<th colspan="5" class="aright"><?php printf( __( "Applies to %s%%", 'usces' ), $usces_tax->tax_rate_standard ); ?></th>
			<th id="subtotal_standard" class="aright">&nbsp;</th>
			<th colspan="2"><input type="hidden" name="order_subtotal_standard" id="order_subtotal_standard" value="<?php if( !empty( $order_subtotal_standard ) ) { usces_crform( $order_subtotal_standard, false, false, '', false ); } else { echo '0'; } ?>">&nbsp;</th>
...
<?php endif; ?>
		<tr>
			...
			<th colspan="2"><input name="recalc" id="recalc" class="button" type="button" value="<?php _e( 'Recalculation', 'usces' ); ?>" /><?php echo $change_taxrate; ?></th>
		</tr>
		</tfoot>
</table>
<?php
...
<?php if( $reduced_taxrate ): ?>
<div class="order_edit_notice_reduced_tax_rate"><span class="reduced_taxrate_mark"><?php echo $usces_tax->reduced_taxrate_mark; ?></span><?php _e( ' is reduced tax rate', 'usces' ); ?></div>
<?php endif; ?>
