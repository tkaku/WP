...
function usces_pdf_out( $pdf, $data ) {
	...
	switch( $_REQUEST['type'] ) {
		...
		default:
			$title = apply_filters( 'usces_filter_pdf_title', $_REQUEST['type'] );
			$pdf->SetTitle( $title );
			$filename = trim( $_REQUEST['type'] ) . '_' . usces_get_deco_order_id( $data->order['ID'] ) . '.pdf';
	}
	...
	$count_cart = count( $usces_tax->cart_standard ) + count( $usces_tax->cart_reduced );

	if( 0 < $usces_tax->subtotal_standard ) {
		...
		foreach( $usces_tax->cart_standard as $cart_row ) {
			...
			if( $onep < $next_y && 0 < $index && $index != $count_cart ) {
	...
	if( 0 < $usces_tax->subtotal_reduced ) {
...
function usces_pdfSetHeader( $pdf, $data, $page, $font ) {
	...
	// Order No.
	$pdf->SetXY( 131, 15 );
	$pdf->MultiCell( 36, $lineheight, usces_get_deco_order_id( $data->order['ID'] ), 0, 'R' );
...
function usces_pdfSetDetail( $pdf, $data, $page, $font, $x, $y, $onep, $next_y, $border, $index, $cart, $cart_row, $fontsizes, $mark = '' ) {
	...
	$pdf->MultiCell( 84.6, $lineheight, usces_conv_euc( apply_filters( 'usces_filter_cart_item_name_nl', $cartItemName.$mark, $args ) ), 0, 'L' );

	if( 'receipt' != $_REQUEST['type'] && !empty( $optstr ) ) {
	...
	$pdf_args = compact( 'page', 'x', 'y', 'onep', 'next_y', 'border', 'index', 'cart_row', 'fontsize', 'lineheight', 'linetop', 'font' );
...
function usces_pdfSetFooter( $pdf, $data, $font, $usces_tax ) {
	...
	$fontsizes = array( 'footer_label'=>9, 'footer_value'=>8, 'footer_note'=>8 );
		...
		if( 'activate' == $member_system && 'activate' == $member_system_point ) {
			if( 1 == $point_coverage ) {
				if( 'products' == $tax_target ) {
					$label_data[] = apply_filters( 'usces_filter_pdf_tax_label', apply_filters( 'usces_filter_tax_label', usces_tax_label( $labeldata, 'return' ) ) );
	...
	$line_y = $y;
	$pdf->SetXY( 16, $line_y );
	$pdf->MultiCell( 86.8, $lineheight, usces_conv_euc( apply_filters( 'usces_filter_pdf_note', $note_text, $data, $_REQUEST['type'] ) ), 0, 'J' );
	...
	$line_y += 2;
	$pdf->SetXY( 15, $line_y );
	$pdf->MultiCell( 40, $lineheight, usces_conv_euc( $usces_tax->reduced_taxrate_mark.__( ' is reduced tax rate', 'usces' ) ), 0, 'L' );
