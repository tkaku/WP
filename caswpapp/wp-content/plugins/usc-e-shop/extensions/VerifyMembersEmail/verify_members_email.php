...
?>
	...
	<div class="postbox">
		<h3 class="hndle" id="verifyemail"><span><?php _e('Verify New Member Email','usces'); ?></span><?php echo $status; ?></h3>
		<div class="inside">
		<table class="form_table">
			<tr height="35">
				<th class="system_th"><a style="cursor:pointer;" onclick="toggleVisibility('ex_verifyemail_switch_flag');"><?php _e('Verifying e-mail', 'usces'); ?></a></th>
				<td width="10"><input name="verifyemail_switch_flag" id="verifyemail_switch_flag0" type="radio" value="0"<?php if(self::$opts['switch_flag'] === 0) echo 'checked="checked"'; ?> /></td><td width="100"><label for="verifyemail_switch_flag0"><?php _e('disable', 'usces'); ?></label></td>
				<td width="10"><input name="verifyemail_switch_flag" id="verifyemail_switch_flag1" type="radio" value="1"<?php if(self::$opts['switch_flag'] === 1) echo 'checked="checked"'; ?> /></td><td width="100"><label for="verifyemail_switch_flag1"><?php _e('enable', 'usces'); ?></label></td>
				<td><div id="ex_verifyemail_switch_flag" class="explanation"><?php _e("Send an e-mail to the e-mail address registered when new member registration, and make them approve that the e-mail address is definitely their own.", 'usces'); ?></div></td>
			</tr>
		</table>
		<hr />
		<input name="usces_verifyemail_option_update" type="submit" class="button button-primary" value="<?php _e('change decision','usces'); ?>" />
		</div>
	</div><!--postbox-->
...
<?php
	...
	***********************************/
	...
	public function send_verifymail($user) {
		...
		$subject = __('Request email confirmation', 'usces');
	...
	***********************************/
	public function get_verify_url($user) {
		...
		$query = array( 'verify' => $encrypt_value, 'page' => 'memberverified' );
	...
	***********************************/
	public function encrypt_value($user) {
		...
		if( 'newmemberform' == $user['regmode'] ){

			$datatext = $user['regmode'] . ',' . $user['ID'];

		}elseif( 'newmemberfromcart' == $user['regmode'] ){

			$datatext = $user['regmode'] . ',' . $user['ID'] . ',' . $user['flat_pass'];
	...
	***********************************/
	public function verified_email() {
		...
		usces_register_action('page_allow_email', 'get', 'page', 'memberverified', array( $this, 'allow_email'));
	...
	***********************************/
	public function allow_email() {
		...
		if( 'newmemberform' == $regmode ){
			...
			$email = $user['mailaddress1'];
			$usces->member_just_login($email, $pass);
	...
	* Modified:30 Sep.2019
	***********************************/
	static function get_remaining_hour(){
	...
	***********************************/
	public function delete_flag(){
		...
		$mem_ids = $wpdb->get_col( $wpdb->prepare( 
			"SELECT `member_id` FROM {$table} WHERE `meta_key` = %s ", '_verifying' ) );
