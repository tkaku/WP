...
<?php
	...
	function member_check_fromcart() {
		...
		if ( WCUtils::is_blank($_POST["customer"]["zipcode"]) && usces_is_required_field('zipcode') ){
			$mes .= __('postal code is not correct', 'usces') . "<br />";
		}
	...
	function delivery_check() {
		...
		if ( isset($_POST['delivery']['delivery_flag']) && $_POST['delivery']['delivery_flag'] == 1 ) {
			...
			if ( WCUtils::is_blank($_POST["delivery"]["zipcode"]) && usces_is_required_field('zipcode') ){
				$mes .= __('postal code is not correct', 'usces') . "<br />";
			}
	...
	function get_skus( $post_id, $keyflag = 'sort' ) {
		...
		foreach( $metas as $rows ){
			...
			$skus[$key] = array(
								...
								'stock' => $values['stock'],
								'gp' => $values['gp'],
								'taxrate' => ( isset( $values['taxrate'] ) ) ? $values['taxrate'] : '',
								'sort' => $values['sort'],
